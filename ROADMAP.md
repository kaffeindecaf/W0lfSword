...
- [ ] `AUD.12` — the STATS table at the bottom of this file has been stale since ~2026-08-13 and now undercounts by a wide margin: its TOTAL reads 290 items / 150 done / 140 remaining, while the file actually holds 376 checkbox rows (248 done / 128 open). Per-section examples: K1 says 11/4/7 but holds 12 rows / 9 done, K4 says 13/2/11, A3 says 22/10/12. Regenerate the table from the file (or drop it in favour of `grep -c` in the audit) - a status table nobody maintains is worse than no table, because it reads as an authoritative progress figure.
