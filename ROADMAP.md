...
- [x] `K1.7` 🟡 — Auto-select best exploit per device in adderall  
  ...
  _Done 2026-09-25 (host-only, no device attached): the auto-select half was already the shared `select_exploit()` that adderall, deploy, tweaks, nojailbreak and status all read, so this closed the two halves the prompt asked for next - the version half of "best exploit", and the route out of "unsupported"._
  _Version-gated verdict: `exploit_method_gap()`, `exploit_method_rows()` and `exploit_method_valid()` now take an optional iOS version, and the kfd PUAF rows are judged against the window the K5 matrix gives kfd (`matrix_row kfd` = 16.0-16.9, read and never retyped). An A9/A10 device - whose iOS tops out at 15.x and therefore can never sit inside a 16.x window - is dimmed as `kfd PUAF is iOS 16.0-16.9 only`, with a footer that says no kernel method is left, instead of claiming "family matches, kfd backend not ported" and calling kfd PUAF "this SoC's path" (what the model-only verdict did on an iPhone8,1/15.8). adderall's picker and its loud validator both get the detected version, so the two surfaces still cannot disagree; A11 on 16.x and every pe_v1/pe_v2 device are unchanged. New `kfd_window_gap()` is the one place that asks the window question, so the row verdict and the picker footer cannot drift._
  _The unknown-SoC route: new `unknown_soc_route()` prints the commands that identify an unmapped SoC's offsets, picked by what is installed here (`~/Desktop/kernel-deltas` xpf-cli present -> `experimental offsets <ipsw-url>`; else `kernelcache extract` + `kernelcache resolve`), and is called from every dead end that used to stop at a bare error: adderall's Phase 2 gate (a confirm-gated offer now, not a silent fall back to 'auto'), `deploy`'s compat gate, `tweaks install`'s gate and the picker's unknown footer. Two wrong messages died with it: the tweak gate blamed "A10-A11 need the PUAF port" for a device in the branch where A10/A11 are NOT the cause, and the footer advertised `kernelcache (pull + XPF)` - "pull" has never been a subcommand of cmd_kernelcache, so the only hint that existed for an unmapped device led straight into a usage error._
  _Guards: new `scripts/check_route_commands.py` resolves every `./W0lfSword <cmd> [<sub>]` the route prints against the CLI itself (registry row for the command, case arm for the subcommand, following one delegation hop such as cmd_experimental -> experimental_body), with a 6-mutation `--selftest` that catches the K1.7 bug itself (`alpha pull`), an unknown command, and a sub that only exists one level up. `scripts/test_exploit_choice.sh` grew 35 -> 49 checks: the version grid (A9/15.8 dimmed with the row's own window, A11/16.5.1 unchanged, pe_v1/26.0.1 unaffected), the window read independently out of `exploit_matrix()` so a row edit the hint does not follow fails, the cross-check that the render dims puaf exactly when the K5.9 selector refuses its kfd row, both route shapes (the second forced by an empty HOME), and the verb check. Docs: `exploits <model> [ios]` in the registry row, the help table, README and the explain text, which also documents the route._
  _Verified: `bash -n`, shellcheck clean, `./W0lfSword audit` PASSED (185 defs / 0 dead, 78 shell+python files parse, 52 commands / 0 consistency findings), `test_exploit_choice.sh` 49/49, `check_route_commands.py --selftest` 6/6, `scripts/regression.sh` 20 passed / 0 failed, `scripts/check_host_verification.sh` 19 ok / 0 drift. The two device-driven gates were run end to end against mock `idevice_id`/`ideviceinfo`/`ssh` shims reporting Bogus9,9 on 26.0.1: adderall printed the refusal + route + "Nothing was deployed." (rc=1) and `tweaks install five_icon_dock` refused the same way before any build (rc=1). Mutation-tested 5 ways (renamed hint verb, kfd row range, dropped window check, window read pointed at a missing row, old footer text restored: 2/2/2/3/5 checks fail respectively, file restored byte-identical 173f389d...). Audit + suite re-run on the restored file._

- [x] `K1.8` 🟡 — Menu shows compatibility per exploit and greys out unsupported choices  
  ...
  _Done 2026-09-25 (host-only, no device): new `exploit_method_gap()` is now the ONE place that decides whether a method pick can run on a model - it prints the reason and returns 1, prints nothing and returns 0 when the pick works, and `exploit_method_valid()` renders its warning from that same string, so the loud validator and the dimmed list can no longer disagree. That mismatch was real: the validator's comment said the puaf picks were "A10-A11 only" while `select_exploit()`'s own `iPhone8,*` row is A9, i.e. two case statements holding the same family rules and already at different versions. New `exploit_method_rows()` renders the annotated list - the method the model's SoC selects marked `<- this device`, every pick that cannot run dimmed with its reason, and three separate footers because the next step differs: MTE (userspace only), A9-A11 (kfd puaf is this SoC's path but the backend is unported, F1.x / K5.3), unknown model (identify the SoC via kernelcache + XPF first). Note the row text is honest about puaf: family matches, backend not ported - never "runnable". Two surfaces, one implementation: adderall's picker renders the block BEFORE the question (to stderr, like every other prompt in that phase, so `EXPLOIT_METHOD=$(ask_choice ...)` still captures only the value) and the existing `exploits` command now takes its first argument - `./W0lfSword exploits iPhone14,7` prints the same block under the matrix, no argument keeps the plain matrix and advertises the form. Reachability: adderall's picker only fires when `select_exploit()` has no mapping (K1.7 auto-selects on a known SoC), which is exactly the unknown-SoC case the new footer addresses - so the verdict is also reachable on demand from the CLI instead of only existing inside a prompt almost nobody sees. Docs: explain `exploits` gained the per-device paragraph, the help table row and the README command row mention `exploits <model>`. Verified: bash -n, shellcheck clean (repo exclusions), `scripts/test_exploit_choice.sh` 35/35 - a 15-model x 6-method grid, row coverage, the picker's accepted values must all be listed, the no-model path, junk-model robustness, and a cross-check that the render and the K5.9 selector agree (pe_v1/pe_v2 marked <-> the matching DarkSword row, A9-A11 <-> no DarkSword row, MTE <-> no chain + MTE reason) - wired into `scripts/regression.sh`, and mutation-tested 5 ways (family rule, reason text, marker logic, MTE footer, model argument: each fails 1-28 checks, restore byte-identical a915e3b0...). Audit PASSED (shellcheck clean, 183 defs / 0 dead, 77 shell+python files parse, 52 commands / 0 consistency findings). Host verification only - no adderall run on hardware, so the device-detection half of the picker is covered by the render grid through the CLI, not by a live run._
...
> **Generated** from the checkbox rows in this file - do not hand-edit. The
> table covers every section that holds an item; `**TOTAL**` is the file's item
> count, so a section the table omits cannot hide items. Regenerate with
> `python3 scripts/roadmap_stats.py --write ROADMAP.md`; `./W0lfSword audit`
> fails while the two disagree (AUD.12).

| Section | Total Items | Completed | Remaining |
|---------|------------|-----------|-----------|
| 0.1 — Quick wins (under 30 min each, no hardware needed) | 5 | 5 | 0 |
| 0.2 — Device/research work (needs iOS 26.1+ hardware or kernelcache) | 6 | 4 | 2 |
| 0.3 — High-value features (userspace direction) | 4 | 4 | 0 |
| 0.4 — Framework research campaign (2026-08-29, audio + media) | 6 | 6 | 0 |
| 0.5 — CVE hunting + attack chains campaign (2026-08-29, 26.1+ direction) | 5 | 5 | 0 |
| 0.6 — PoC lab campaign (2026-08-29, test the found bugs) | 6 | 6 | 0 |
| 0.7 — Full audit + adderall hardening + experimental (2026-08-29) | 7 | 7 | 0 |
| 0.8 — MobileGestalt editing round (2026-09-02, Chain F kernel-route impl) | 5 | 5 | 0 |
| 0.9 — SpringBoard live tweaks round (2026-09-02, TaskRop RemoteCall driver) | 5 | 5 | 0 |
| 0.10 — Staged auto-mode exploit (2026-09-02, main-device safety) | 10 | 6 | 4 |
| 0.12 — Open bug list (from the 2026-09-11 device day) | 6 | 6 | 0 |
| 0.11 — Terminal with full kernel R/W (research, 2026-09-10) | 6 | 1 | 5 |
| A1 — Thread Safety | 14 | 14 | 0 |
| A2 — Filza Compatibility | 11 | 9 | 2 |
| A5 — SSV & Sandbox Escape Stability (audit 2026-08-10) | 10 | 10 | 0 |
| A6 — Production Readiness (audit 2026-08-10) | 21 | 21 | 0 |
| A3 — Kernel Exploit Robustness | 22 | 22 | 0 |
| B1 — Multi-App Support | 4 | 0 | 4 |
| B2 — Runtime Control | 3 | 1 | 2 |
| B3 — Power User Features | 7 | 0 | 7 |
| B4 — Developer / Debug Features | 4 | 0 | 4 |
| C1 — iOS Kernel Vulnerability Hunting | 7 | 1 | 6 |
| C2 — Apple Bug Bounty: Specific Targets | 8 | 1 | 7 |
| C3 — Practical Attack Chains | 4 | 0 | 4 |
| C4 — Framework Attack Surface: Audio & Media (campaign 2026-08-29) | 7 | 7 | 0 |
| C5 — CVE Hunting + Attack Chains (campaign 2026-08-29, 26.1+ direction) | 5 | 5 | 0 |
| C6 — PoC Lab (campaign 2026-08-29, test the found bugs) | 6 | 6 | 0 |
| D1 — Refactoring | 6 | 4 | 2 |
| D2 — Testing Infrastructure | 4 | 2 | 2 |
| D3 — Documentation | 3 | 3 | 0 |
| E: iOS Version / Device Expansion | 5 | 0 | 5 |
| F1 — PUAF as Fallback Exploit | 4 | 0 | 4 |
| F2 — Cross-Process Injection (opainject-style) | 3 | 0 | 3 |
| F3 — Standalone App (TrollStore-style) | 3 | 0 | 3 |
...
| J2 — Profile System | 3 | 3 | 0 |
| J3 — Device Manager | 2 | 2 | 0 |
| J4 — Live Exploit Monitoring | 2 | 2 | 0 |
| J5 — Exploit History & Stats | 2 | 2 | 0 |
| J6 — Original W0lfSword Commands (all preserved) | 4 | 4 | 0 |
| J7 — Polish | 3 | 2 | 1 |
| J8 — Beta UX (newly added 2026-08-10) | 6 | 6 | 0 |
| K1 — Exploit Menu (multi-exploit selection) | 12 | 10 | 2 |
| K2 — Beginner-Friendly Reform | 10 | 9 | 1 |
| K3 — Tweak Menu (choose a tweak from the CLI and install it) | 9 | 4 | 5 |
| K4 — iOS 26.1 Sandbox Escape Research (see referenceforAI/SandboxEscape.md) | 14 | 7 | 7 |
| K5 — Exploit Chains (recipes to add) | 9 | 3 | 6 |
| L1 — Concept & Scope | 4 | 0 | 4 |
| L2 — App Shell & Packaging | 6 | 5 | 1 |
| L3 — Reuse the W0lfSword Engine | 5 | 2 | 3 |
| L4 — Sandbox Escape Testing | 4 | 0 | 4 |
| L5 — Privilege Escalation | 5 | 2 | 3 |
| L6 — Exploit Hub Features | 4 | 0 | 4 |
| L7 — Safety & Persistence | 4 | 1 | 3 |
| L8 — Testing & Release | 4 | 0 | 4 |
...
- [x] `AUD.4` — CVE catalog lives in two places: cmd_cve tables + research/attack_chains.md + kernel26_cves.md. One source of truth (a TSV data file) would stop drift. Medium.
  _Done 2026-09-24: the rows moved out of cmd_cve into **research/cve_catalog.tsv** (32 rows, fields `id|class|component|fixed_in|live_on|groups`: groups = the `cve <filter>` names, comma separated, plus `live`), and `cmd_cve` now renders the file through one awk group-filter instead of four hand-kept case blocks. That is where the drift was: `cve all` was a separately maintained shortlist, **14 rows behind** the filters (no CVE-2026-20626/28972/39868/43805/64751, CVE-2025-43407/43448, CVE-2026-20628/28995, BB-032, CVE-2025-43530, CVE-2026-20677, BB-007, BB-003). `all` is now the whole catalog in file order, so the file IS the display order; the per-group tables are byte-identical to before (diffed against `git show HEAD:W0lfSword` for kernel/userspace/sandbox/tcc/ssv), `live` keeps the same 9 rows (new order), and the `k`/`u`/`s` aliases plus `LIVE` now print the canonical filter name in the header. Guards so it cannot drift back: (1) `scripts/cli_consistency.py` (so every `./W0lfSword audit`) checks row shape (6 fields, no empty field, unique id), group names, that every group has rows, that cmd_cve still names *and reads* `research/cve_catalog.tsv`, that no catalog row is inlined in the script again, and that every id has a write-up somewhere in `research/ pocs/ docs/ scripts/ README.md ROADMAP.md CONTEXT.md BUG_BOUNTY.md` - proven with 11 mutations (missing file, 4-field row, duplicate id, unknown group, empty field, emptied group, orphan id, row inlined back, path renamed, path assigned-but-unread, cmd_cve gone), each flagged, none on the clean tree; (2) `scripts/test_cve_catalog.sh` (15 checks, wired into `scripts/regression.sh`) renders every filter through the CLI and compares the ids to an independent awk read of the TSV in file order, plus the alias/case equivalence, the anti-inline grep and the missing-file path (a CLI copy run from a directory with no `research/` must refuse with a hint, not print an empty table) - negative-tested 5 ways (inlined row 8 failures, filter that unions the live rows 5, `all` shortlist restored 1, missing-file guard removed 1, rows no longer read from the file 7). Also fixed a real trap this created: `GROUPS` is a bash special (the caller's group ids) and read-only, so naming the test's filter array `GROUPS` silently loops over numbers - both sides compared empty and passed; renamed to `CVE_GROUPS`. Docs: `research/attack_chains.md` (local-only, gitignored) + `research/kernel26_cves.md` (tracked) now point at the TSV as the machine-readable catalog, the README command table needed no change (the filter names are unchanged). Known limitation of the traceability check, stated rather than hidden: its evidence set is the working tree, and two rows (CVE-2025-43448, CVE-2026-28995) are written up only in `research/userspace_escapes.md` (gitignored), so a fresh clone's audit flags those two until that doc is committed. Verified: audit PASSED (shellcheck clean, 181 defs/0 dead, 76 shell/python files parse, cli_consistency 52 commands/39 sections/10 matrix rows/32 catalog rows/0 findings), `bash -n` on the CLI + the new script, `cve all|kernel|k|live|LIVE|ssv` renders, catalog test 15/15, negative tests as above. One shellcheck finding on the first run (SC2034, the row loop's 6th field read into an unused `groups` var) fixed by reading it into `_`._
...
- [x] `AUD.11` - host verification suite: a host change must not read as drift (the trm_shell pin)
  _Done 2026-09-24: `scripts/check_host_verification.sh` came up 17 ok / 1 drift with no harness change behind it (`git diff f999d55..HEAD -- terminal tests/trm_shell_host_test.c scripts/run_trm_host_test.sh` is empty). The gap was host identity: the Parrot kernel update of 2026-09-22 (7.0.13 -> 7.1.13) moved the one line the canon mask did not cover, `uname | [sh] Linux 7.0.13+parrot7-amd64 ...`. Proved rather than assumed: today's log through the OLD mask with only that string put back to its pre-update value reproduces the old pin byte-for-byte (89e5f65f...). Fixed at the mask, not at the pin - `CANON_SED` now also masks the kernel string, the caller's uid/euid/gid/egid, SHELL and the machine type `cpu` prints, and the new `canon-trm` mode drops the compiler's diagnostic preamble (run_trm_host_test.sh compiles with `$CC` every run, so gcc's wording is the toolchain's evidence, not this tree's; rc proves the compile). New `canon_mask` check pins the claim from both sides and is negative-tested: deleting the kernel-string rule fails it and the pin (17 ok / 2 drift), deleting the creds rule fails it. Re-pin: `trm_shell_host_test` canon hash 89e5f65f... -> 15062e05..., stable over three runs and invariant to the kernel string (identical hash with either string in the log). Same file, second fix: `engine_lib_archive` printed the pre-BUG.7 hash as its "want" while comparing against the live one, so a real drift would have reported the wrong expected value - rebuilt (`make libengine` -> 808K / 53 objects, 9acea996...) to confirm which is live and made the failure path print that same value. Verified: suite 19 ok / 0 drift twice, `canon-trm` drops exactly the 14 preamble lines and nothing else (body + `CANON_SED` == log + `CANON_SED_TRM`), shellcheck findings unchanged (3 pre-existing SC2016 infos), bash -n, ./W0lfSword audit PASSED._
- [x] `AUD.12` — the STATS table at the bottom of this file has been stale since ~2026-08-13 and now undercounts by a wide margin: its TOTAL reads 290 items / 150 done / 140 remaining, while the file actually holds 376 checkbox rows (248 done / 128 open). Per-section examples: K1 says 11/4/7 but holds 12 rows / 9 done, K4 says 13/2/11, A3 says 22/10/12. Regenerate the table from the file (or drop it in favour of `grep -c` in the audit) - a status table nobody maintains is worse than no table, because it reads as an authoritative progress figure.
  _Queued 2026-09-25 (found while ticking K1.7). Not touched here: the drift is repo-wide, so fixing one row would only move the inconsistency._
  _Done 2026-09-26: the table is generated from the file now, and the audit fails while the two disagree - the generated version reads 377 items / 249 done / 128 open over 63 sections (was 290/150/140 over 41), every row taken from the file's own headers (the `K` aggregate row and the `—` rows for I1/I2 are gone because no section is hand-listed any more). New `scripts/roadmap_stats.py` owns both halves: default = check, `--write` = regenerate, `--json`, `--selftest`. Groups are the file's `## ` headers (items under a level-1 header with no `## ` parent, i.e. SECTION E's E1.*, fall back to that header's name), checkboxes inside a fenced block are not items, and only groups that hold an item get a row while `**TOTAL**` stays the file's item count - so a section the table omits cannot hide items. Wired into `cmd_audit` as another optional step (`scripts/roadmap_stats.py` missing = skipped with a hint): findings count toward the verdict, `audit --json` gained a `roadmap_stats` key (check/items/rows/findings/ok), and the failure path prints the offending line plus `regenerate: python3 scripts/roadmap_stats.py --write ROADMAP.md`. The check is proven able to fail - 11 fixture mutations in `--selftest`, incl. THE drift shape (a checkbox ticked, table untouched) plus a hand-edited count, a deleted row, a stale extra row, a zero-item row, a fenced checkbox, a missing table (exit 2) and `--write` leaving every non-table line byte-identical; `scripts/regression.sh` runs the selftest beside the other host lints. Verified: `--write` run twice on the tree (second run byte-identical = idempotent), check `findings: 0`, mutation test on the live ROADMAP.md (one count edited by hand) made both the check and the audit fail with exactly 1 issue and `issues: 1 / passed: false` in `audit --json`, then restored from the pre-mutation snapshot (sha256 60424bd3... reproduced, so the mutation left nothing behind), `bash -n` OK, shellcheck clean, audit PASSED. The STATS note above the table says it is generated and names the regenerate command, so the next session does not hand-edit it again._
