...
- [x] `K4.1` 🔴 — Verify 26.1 kernel struct offsets (sandbox, MACF label) vs 26.0.1 — pull 26.1 kernelcache, run XPF, add offsets.m block if shifted
  ...
  _Follow-up 2026-08-24: A13/t8030 18.4.1 (SE2) kc resolved — task.itk_space 0x318 confirms A13, per-SoC flag resolved (T8150/A18-only at 0x310). Details in Section K4._
...
- [x] `K4.1` 🔴 — Verify 26.1 kernel struct offsets (sandbox, MACF label) vs 26.0.1  \
  ...
  _Follow-up 2026-08-24 (SE 2nd gen, A13/t8030, 18.4.1): pulled kernelcache.release.iphone12c from the 22E252 IPSW with the new ranged fetcher (scripts/fetch_kernelcache.py — zip64 EOCD + zip64 local-header handling, ~19MB of an 8.45GB IPSW) and ran xpf-cli on it. Matches the running kernel (xnu-11417.102.9~20). RESOLVED the flag: task.itk_space = 0x318 on A13 too, so A13+A15 = 0x318 and the 0x310 delta is T8150/A18-only — offsets.m 18.x stays 0x318 (comment updated). Also: machine_kstackptr 0xf8 (matches isA13Above), proc.struct_size 0x740 vs 0x748 on 26.x (grew 8 bytes — kcwatch signal), machine_CpuDatap 0x148 (SoC delta), sptm=0 on 18.4.1 vs 1 on 26.x (SPTM arrived between 18.4.1 and 26.0). Full table in tools/xpf-cli/README.md._
