...
TOOLS_DIR = Path(__file__).parent.parent.parent / ".w0lfsword" / "usbliter8" / "tools"
DRY_RUN = False
VERBOSE = True
...
def patch_ibss(ipsw_dir: str | Path, offsets: dict, work_dir: str | Path) -> bool:
    ...
    src = Path(ipsw_dir) / "Firmware" / "dfu" / "iBSS.d421.RELEASE.im4p"
    if not src.exists():
    ...
    dest = Path(ipsw_dir) / "Firmware" / "dfu" / "iBSS.d421.RELEASE.im4p"
    return _wrap_raw_to_im4p(raw, dest)
...
def patch_ibec(ipsw_dir: str | Path, offsets: dict, work_dir: str | Path) -> bool:
    ...
    src = cfw_dir / "iBEC.d421.RELEASE.im4p"
    if not src.exists():
        # fallback: try in ipsw dir
        src = Path(ipsw_dir) / "Firmware" / "dfu" / "iBEC.d421.RELEASE.im4p"
...
def patch_devicetree(ipsw_dir: str | Path, offsets: dict, work_dir: str | Path) -> bool:
    ...
    src = Path(ipsw_dir) / "Firmware" / "all_flash" / "DeviceTree.d421ap.im4p"
    if not src.exists():
    ...
    dt_scripts_dir = Path(__file__).parent.parent.parent / "referenceforAI" / "usbliter8-fun2" / "work-27.0b2"
    scripts = [
    ...
    dest = Path(ipsw_dir) / "Firmware" / "all_flash" / "DeviceTree.d421ap.im4p"
...
def patch_restoreramdisk(ipsw_dir: str | Path, offsets: dict, work_dir: str | Path) -> bool:
    ...
    if not _extract_im4p_to_raw(ramdisk_path, raw):
        print(err("Failed to extract RestoreRamdisk — attempting direct patch"))
        raw = ramdisk_path

    with open(raw, "r+b") as fp:
        _apply_dict_patches(fp, rd_patches, "restoreramdisk")

    if DRY_RUN:
        return True

    print(ok("RestoreRamdisk patches applied"))
    return True
...
def patch_userland(ipsw_dir: str | Path, offsets: dict, work_dir: str | Path) -> bool:
    ...
    for daemon_name, patches in daemon_patches.items():
        ...
        if not binary_path:
            print(warn(f"{daemon_name} binary not found in IPSW — skipping"))
            continue
